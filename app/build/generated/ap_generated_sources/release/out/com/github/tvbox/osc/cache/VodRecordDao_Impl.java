  ...
  public VodRecordDao_Impl(RoomDatabase __db) {
    ...
    this.__insertionAdapterOfVodRecord = new EntityInsertionAdapter<VodRecord>(__db) {
      ...
      public String createQuery() {
        return "INSERT OR REPLACE INTO `vodRecord` (`vodId`,`updateTime`,`sourceKey`,`dataJson`,`id`) VALUES (?,?,?,?,nullif(?, 0))";
      }
      ...
      public void bind(SupportSQLiteStatement stmt, VodRecord value) {
        if (value.vodId == null) {
          ...
          stmt.bindString(1, value.vodId);
        }
        stmt.bindLong(2, value.updateTime);
        if (value.sourceKey == null) {
          stmt.bindNull(3);
        } else {
          stmt.bindString(3, value.sourceKey);
        }
        if (value.dataJson == null) {
          stmt.bindNull(4);
        } else {
          stmt.bindString(4, value.dataJson);
        }
    ...
    this.__preparedStmtOfDeleteAll = new SharedSQLiteStatement(__db) {
      @Override
      public String createQuery() {
        final String _query = "DELETE FROM vodRecord";
        return _query;
      }
    };
